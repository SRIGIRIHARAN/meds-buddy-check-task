...
const MedicationList = () => {
  ...
  // Helper: Map medication_id to log
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await deleteMedication(id, user!.id);
      if (error) throw error;
    },
    onSuccess: () => {
      toast({ title: "Medication deleted", variant: "default" });
  ...
  return (
    <div className="space-y-4">
      {medications.map(med => {
        ...
        // If marked as taken, show completed card
        if (log && log.taken) {
          return (
        ...
        // Otherwise, show medication row (with edit/delete/mark as taken)
        return (
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      size="icon"
                      onClick={() => {
                        if (window.confirm("Delete this medication?")) deleteMutation.mutate(med.id);
                      }}
